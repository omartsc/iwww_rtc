...
class Example(CSVMixin, ModelicaMixin, CollocatedIntegratedOptimizationProblem):
    ...
    def objective(self, ensemble_member):
        # Maximize water drained through the "orifice"
        ...
        return self.integral('Q_orifice', ensemble_member)
    ...
    def path_constraints(self, ensemble_member):
        ...
        constraints.append((self.state('storage.HQ.H'), 0.2, 0.4))

        constraints.append((self.state('drainage_channel.HQ.H'), 0.0, 0.3))

        constraints.append((self.state('main_channel.HQ.H'), 0.0, 0.5))

        constraints.append((self.state('channel_connect.HQ.H'), 0.0, 0.8))

        # constraints.append((self.state('H_channel'), 0.0, 3.0))

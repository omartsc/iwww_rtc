model Example
  ...
  Deltares.ChannelFlow.SimpleRouting.BoundaryConditions.Terminal drain(Q = Q_in_drain) annotation(Placement(visible = true, transformation(origin = {15, -35}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  // Initial States
  parameter Modelica.SIunits.Volume storage_V_init;
  // Inputs
  input Modelica.SIunits.VolumeFlowRate P_control(fixed = true);
  input Modelica.SIunits.VolumeFlowRate Q_in_drain(fixed = true);
...
equation
  connect(storage.QOut, drain.QIn) annotation(
    Line(points = {{10, 0}, {7, 0}, {7, -35}}));
  connect(inflow.QOut, storage.QIn) annotation(
    Line(points = {{-47, 0}, {-10, 0}}));
  connect(storage.QOut, outfall.QIn) annotation(
    Line(points = {{8, 0}, {47, 0}}));

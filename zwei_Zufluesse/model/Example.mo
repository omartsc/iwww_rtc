model Example
  ...
  Deltares.ChannelFlow.Hydraulic.Storage.Linear TSP(A = 1.0e6, H_b = 0.0, HQ.H(min = 0.0, max = 0.8)) annotation(
  ...
  Deltares.ChannelFlow.Hydraulic.Storage.Linear UWB(A = 1.0e6, H_b = 0.0, HQ.H(min = 0.0, max = 2.0)) annotation(
  ...
  input Modelica.SIunits.VolumeFlowRate Q_KBW_2(fixed = true) = KBW_2.Q;
  input Modelica.SIunits.VolumeFlowRate Q_KBW(fixed = false, min = 6.0, max = 18.0) = KBW.Q;
